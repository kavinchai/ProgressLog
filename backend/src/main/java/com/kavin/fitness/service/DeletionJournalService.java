...
/**
 * Records destructive operations to the deletion journal so they can later be replayed by {@link
 * UndoService}. Each delete service hits this with a JSON snapshot of what's about to be removed.
 */
@Service
...
public class DeletionJournalService {
    ...
    @Transactional
    public DeletionJournalEntry record(
            User user, String entityType, String summary, Object snapshotPojo) {
        String json;
        try {
    ...
    public List<UndoActionDTO> getRecent(Long userId) {
        return repository.findTop20ByUserIdAndRestoredAtIsNullOrderByCreatedAtDesc(userId).stream()
                .map(
                        entry ->
                                new UndoActionDTO(
                                        entry.getId(),
