...
public class SharedCalendarService {
    ...
    public SharedCalendarDTO getCalendar() {
        ...
        List<WorkoutSession> sessions =
                workoutSessionRepository.findByUserIdInWithSetsOrderByDateDesc(
                        new ArrayList<>(userIdToName.keySet()));

        List<SharedCalendarDTO.Entry> entries =
                sessions.stream()
                        .filter(s -> !s.getExerciseSets().isEmpty())
                        .map(
                                s ->
                                        new SharedCalendarDTO.Entry(
                                                userIdToName.getOrDefault(
                                                        s.getUser().getId(), "unknown"),
                                                s.getSessionDate(),
                                                s.getSessionName(),
