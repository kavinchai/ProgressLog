...
public interface DeletionJournalRepository extends JpaRepository<DeletionJournalEntry, Long> {
    ...
    Optional<DeletionJournalEntry> findFirstByUserIdAndRestoredAtIsNullOrderByCreatedAtDesc(
            Long userId);

    /** Most recent N un-restored entries for a user (for showing what's undoable). */
    List<DeletionJournalEntry> findTop20ByUserIdAndRestoredAtIsNullOrderByCreatedAtDesc(
            Long userId);
