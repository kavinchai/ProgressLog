...
public class WorkoutController {
    ...
    public ResponseEntity<List<String>> getExerciseNames(
        ...
        return ResponseEntity.ok(
                workoutService.getDistinctExerciseNames(userResolver.resolve(principal).getId()));
    }
    ...
    public ResponseEntity<WorkoutSessionDTO> getWorkout(
            @AuthenticationPrincipal UserDetails principal, @PathVariable Long sessionId) {
        return ResponseEntity.ok(
                workoutService.getWorkoutSession(
                        sessionId, userResolver.resolve(principal).getId()));
    }

    @GetMapping("/by-exercise/{exerciseName}")
    public ResponseEntity<List<WorkoutSessionDTO>> getWorkoutsByExercise(
            @AuthenticationPrincipal UserDetails principal, @PathVariable String exerciseName) {
        long userId = userResolver.resolve(principal).getId();
    ...
    public ResponseEntity<WorkoutSessionDTO> logWorkout(
        ...
        log.info(
                "POST workout user={} date={} exercises={}",
    ...
    public ResponseEntity<WorkoutSessionDTO> upsertExercise(
        ...
        log.info(
                "POST upsert exercise sessionId={} exercise={}",
                sessionId,
                request.getExerciseName());
        WorkoutSessionDTO dto =
                workoutService.upsertExercise(
