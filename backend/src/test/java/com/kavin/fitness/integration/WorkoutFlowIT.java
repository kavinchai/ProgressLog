...
/**
 * A user logs a workout, adds exercises, renames the session, reads it back, and deletes it. A
 * second user cannot see or modify it.
 */
class WorkoutFlowIT extends IntegrationTestBase {
    ...
    void logWorkout_readBack_rename_delete() throws Exception {
        ...
        mockMvc.perform(
                        patch("/api/workouts/" + sessionId + "/name")
                                .header("Authorization", "Bearer " + token)
                                .contentType(MediaType.APPLICATION_JSON)
    ...
    void otherUser_cannotSeeOrDeleteWorkout() throws Exception {
        ...
        mockMvc.perform(get("/api/workouts").header("Authorization", "Bearer " + tokenB))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(0));
