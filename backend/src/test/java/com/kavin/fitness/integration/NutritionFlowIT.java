...
/**
 * A user creates a nutrition day, adds meals, sees totals aggregate, updates a meal, deletes a
 * meal, and deletes the day. A second user's data is isolated.
 */
class NutritionFlowIT extends IntegrationTestBase {
    ...
    void fullNutritionDayFlow() throws Exception {
        ...
        long logId =
                objectMapper
                        .readTree(dayResult.getResponse().getContentAsString())
                        .get("id")
                        .asLong();
        ...
        mockMvc.perform(
                        put("/api/nutrition/" + logId + "/meals/" + mealId)
                                .header("Authorization", "Bearer " + token)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(updatedMeal))
                .andExpect(status().isOk())
