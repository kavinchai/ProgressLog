...
/**
 * Exercises the deletion-journal / undo flow end-to-end for every MCP-exposed destructive op. Each
 * scenario: log → delete → undo → verify the data is back. IDs are allowed to change on restore.
 */
class UndoFlowIT extends IntegrationTestBase {
    ...
    void undo_restoresDeletedWorkoutSession_withExercises() throws Exception {
        ...
        // Undo
        mockMvc.perform(post("/api/undo").header("Authorization", "Bearer " + token))
    ...
    void undo_restoresDeletedExerciseSets_withinSession() throws Exception {
        ...
        mockMvc.perform(
                        delete("/api/workouts/" + sessionId + "/exercises")
                                .header("Authorization", "Bearer " + token)
                                .param("name", "Deadlift"))
                .andExpect(status().isNoContent());
    ...
    void undo_restoresDeletedNutritionLog_withMeals() throws Exception {
        ...
        MvcResult logRes =
                mockMvc.perform(
                        ...
                        .andReturn();
        long logId =
                objectMapper.readTree(logRes.getResponse().getContentAsString()).get("id").asLong();

        // Add two meals
        mockMvc.perform(
                        post("/api/nutrition/" + logId + "/meals")
                                .header("Authorization", "Bearer " + token)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(
                                        """
        ...
        mockMvc.perform(get("/api/nutrition").header("Authorization", "Bearer " + token))
                .andExpect(jsonPath("$.length()").value(0));
    ...
    void undo_restoresDeletedMeal() throws Exception {
        ...
        // Find the meal id
        JsonNode meals =
                objectMapper.readTree(mealRes.getResponse().getContentAsString()).get("meals");
        long mealId = meals.get(0).get("id").asLong();

        // Delete that meal
        mockMvc.perform(
                        delete("/api/nutrition/" + logId + "/meals/" + mealId)
                                .header("Authorization", "Bearer " + token))
                .andExpect(status().isNoContent());
    ...
    void undo_restoresDeletedStepLog() throws Exception {
        ...
        mockMvc.perform(delete("/api/steps/" + stepId).header("Authorization", "Bearer " + token))
                .andExpect(status().isNoContent());

        mockMvc.perform(get("/api/steps").header("Authorization", "Bearer " + token))
                .andExpect(jsonPath("$.length()").value(0));
    ...
    void undo_restoresDeletedWeightLog() throws Exception {
        ...
        mockMvc.perform(get("/api/weight").header("Authorization", "Bearer " + token))
                .andExpect(jsonPath("$.length()").value(0));
    ...
    void undo_popsMostRecentAction_only() throws Exception {
        ...
        MvcResult w =
                mockMvc.perform(
                        ...
                        .andReturn();
        long weightId =
                objectMapper.readTree(w.getResponse().getContentAsString()).get("id").asLong();
        mockMvc.perform(
                        delete("/api/weight/" + weightId)
                                .header("Authorization", "Bearer " + token))
                .andExpect(status().isNoContent());
        ...
        MvcResult s =
                mockMvc.perform(
                        ...
                        .andReturn();
        long stepId =
                objectMapper.readTree(s.getResponse().getContentAsString()).get("id").asLong();
        mockMvc.perform(delete("/api/steps/" + stepId).header("Authorization", "Bearer " + token))
                .andExpect(status().isNoContent());
    ...
    void undo_cannotUndoOtherUsersDeletion() throws Exception {
        ...
        mockMvc.perform(post("/api/undo").header("Authorization", "Bearer " + tokenB))
                .andExpect(status().isNotFound());
    ...
    void recentActions_listsUndoableEntries_mostRecentFirst() throws Exception {
        ...
        long weightId =
                objectMapper.readTree(w.getResponse().getContentAsString()).get("id").asLong();
        mockMvc.perform(
                delete("/api/weight/" + weightId).header("Authorization", "Bearer " + token));
        ...
        mockMvc.perform(get("/api/undo/recent").header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
