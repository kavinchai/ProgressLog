...
/**
 * A user reads default goals, updates them, reads them back, updates email, verifies password, and
 * changes credentials.
 */
class ProfileFlowIT extends IntegrationTestBase {
    ...
    void goalsFlow_readDefaults_update_readBack() throws Exception {
        // 1. Default goals
        mockMvc.perform(get("/api/profile/goals").header("Authorization", "Bearer " + token))
    ...
    void emailFlow_readRegistered_update_readBack() throws Exception {
        ...
        mockMvc.perform(get("/api/profile/email").header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
    ...
    void changePassword_oldPasswordStopsWorking_newPasswordWorks() throws Exception {
        ...
        String username =
                objectMapper
                        .readTree(credResult.getResponse().getContentAsString())
                        .get("username")
                        .asText();
        ...
        mockMvc.perform(get("/api/profile/goals").header("Authorization", "Bearer " + newToken))
                .andExpect(status().isOk());
    }
}
