...
class AuthFlowIT extends IntegrationTestBase {
    ...
    void fullAuthFlow() throws Exception {
        ...
        mockMvc.perform(get("/api/profile/goals").header("Authorization", "Bearer " + token2))
                .andExpect(status().isOk());
    }

    @Test
    void unauthenticatedRequest_returns401() throws Exception {
        mockMvc.perform(get("/api/profile/goals")).andExpect(status().isUnauthorized());
    }

    @Test
    void invalidToken_returns401() throws Exception {
        mockMvc.perform(get("/api/profile/goals").header("Authorization", "Bearer fake.token.here"))
