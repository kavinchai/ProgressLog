...
function computeStats(sessions, isRun) {
	...
	if (isRun) {
		...
		const longestDist = Math.max(
			...sessions.map((s) => Number(s.totalDistanceMiles) || 0),
		);
		const totalMiles = sessions.reduce(
			(sum, s) => sum + (Number(s.totalDistanceMiles) || 0),
			0,
		);
	...
	const longestDur = Math.max(
		...sessions.map((s) => Number(s.totalDurationSeconds) || 0),
	);
	const totalDur = sessions.reduce(
		(sum, s) => sum + (Number(s.totalDurationSeconds) || 0),
		0,
	);
							...
							<div className="chart-tooltip">
								...
								<div className="chart-tooltip-value">
									...
									{payload[0].payload.isPR && (
										<span className="chart-pr-tag"> PR</span>
									)}
								</div>
							</div>
						) : null
...
function Stat({ label, value, accent, testId }) {
	return (
		<div className="cardio-stat" data-testid={testId}>
			...
			<span
				className={
					"cardio-stat-value" + (accent ? " cardio-stat-value-accent" : "")
				}
			>
				{value}
			</span>
...
export default function Cardio() {
	...
	const isRun = useMemo(
		() => (activeExercise ? hasDistance(activeExercise.data) : false),
		[activeExercise],
	);
	const stats = useMemo(
		() => computeStats(filteredSessions, isRun),
		[filteredSessions, isRun],
	);
	const weekly = useMemo(
		() => weeklyVolume(filteredSessions),
		[filteredSessions],
	);
							...
							return (
								<li key={item.name}>
									<button
										...
										className={
											"cardio-sidebar-item" +
											(isActive ? " cardio-sidebar-item-active" : "")
										}
										onClick={() => setActiveName(item.name)}
									>
										<span className="cardio-sidebar-item-name">
											{item.name}
										</span>
										<span
											className="cardio-sidebar-item-meta"
											aria-hidden="true"
										>
											{item.meta}
										</span>
					...
					{filteredSessions.length === 0 ? (
						<div className="section-box">
							<div className="section-body cardio-no-data">
								...
								<p className="cardio-empty-hint">
									Try a longer range like 6M or All.
								</p>
							</div>
						</div>
					) : (
						<>
							...
							<div className="section-box">
								<div className="section-header">
									<span className="section-title">
										{isRun ? "Weekly Volume" : "Weekly Time"}
									</span>
							...
							<div className="section-box">
								<div className="section-header">
									<span className="section-title">
										{isRun ? "Pace Trend" : "Duration Trend"}
									</span>
									{isRun && (
										<span
											className="muted"
								...
								{sortedSessions.map((s) => {
									...
									return (
										<li
											key={s.sessionDate}
											className={
												"cardio-session" + (isPR ? " cardio-session-pr" : "")
											}
										>
											<span className="cardio-session-date">
												{s.sessionDate}
											</span>
											<span className="cardio-session-distance">
												{isRun && dist > 0 ? `${dist.toFixed(2)} mi` : ""}
											</span>
											<span className="cardio-session-duration">
												{formatDuration(dur)}
											</span>
