...
export default function Community() {
	...
	useEffect(() => {
		...
		api
			...
			.then((res) => {
				...
				const d = res.data;
				setData(d);
				const allStrength = (d?.exercises ?? []).filter(
					(e) => e.type === "strength",
				);
				...
				if (featured.length) {
					...
					const match = allStrength.find((e) =>
						nameSet.has(e.exerciseName.toLowerCase()),
					);
					if (match) firstStrength = match.exerciseName;
				}
				if (firstStrength) setSelectedStrength(firstStrength);
				const cardioList = (d?.exercises ?? []).filter(
					(e) => e.type === "cardio",
				);
	...
	const activeList =
		exerciseTab === "cardio" ? cardioExercises : strengthExercises;
	...
	return (
		<div className="lb-page">
			{!authenticated && (
				<header className="lb-topbar">
					...
					<div className="lb-auth-actions">
						...
						<Link to="/login?mode=signup" className="lb-btn lb-btn-primary">
							Create Account
						</Link>
					</div>
				</header>
			...
			<section className="lb-hero">
				<h1>
					{view === "calendar" ? "Community Calendar" : "Community Leaderboard"}
				</h1>
				<p>
					{view === "calendar"
			...
			<nav className="lb-subnav">
				<button
					...
					className={
						"lb-subnav-btn" +
						(view === "leaderboard" ? " lb-subnav-btn-active" : "")
					}
				...
				<button
					...
					className={
						"lb-subnav-btn" +
						(view === "calendar" ? " lb-subnav-btn-active" : "")
					}
			...
			{view === "leaderboard" &&
				...
				data.totalUsers === 0 && (
					<div className="lb-empty">
						{authenticated ? (
							<>
								No one has opted in to sharing yet. Toggle <b>Share Data</b> in{" "}
								<Link to="/settings">Settings</Link> to get on the board.
							</>
						) : (
							<>
								No one has opted in to sharing yet. Sign in and toggle{" "}
								<b>Share Data</b> in Settings to get on the board.
							</>
						)}
					</div>
				)}
							...
							<div className="lb-card lb-card-wide">
								...
								<div className="lb-ex-tabs">
									<button
										...
										className={
											"lb-ex-tab" +
											(exerciseTab === "strength" ? " lb-ex-tab-active" : "")
										}
									...
									<button
										...
										className={
											"lb-ex-tab" +
											(exerciseTab === "cardio" ? " lb-ex-tab-active" : "")
										}
										...
										<div className="lb-exercise-list">
											{activeList.map((e) => (
												<button
													...
													className={
														"lb-pill" +
														(e.exerciseName === currentExercise?.exerciseName
															? " active"
															: "")
													}
									...
									<div className="lb-podium">
										{data.topLifters.map((u) => {
											...
											return (
												<div
													key={u.username}
													className={`lb-podium-card lb-podium-rank-${Math.min(u.rank, 4)}`}
												>
													<span className="lb-podium-medal">{medal}</span>
													<span className="lb-podium-user">{u.username}</span>
													<span className="lb-podium-value">
														{Math.round(u.totalVolumeLbs).toLocaleString()} lbs
													</span>
													<span className="lb-podium-date">
														{u.totalSets} sets · {u.sessionCount} sessions
													</span>
												</div>
											);
										})}
									...
									<ResponsiveContainer width="100%" height={200}>
										...
										>
											...
											<XAxis
												...
												interval={Math.max(
													0,
													Math.floor(activityChartData.length / 8),
												)}
												tick={{
													fontFamily: "var(--font)",
													fontSize: 11,
			...
			<footer className="lb-footer">
				{authenticated ? (
					<>
						Want to be on the board? Opt in to data sharing in{" "}
						<Link to="/settings">Settings</Link>.
					</>
				) : (
					<>
						Want to be on the board?{" "}
						<Link to="/login?mode=signup">Create an account</Link>, log
						workouts, and opt in to data sharing in your settings.
					</>
				)}
			</footer>
		</div>
...
function ExerciseBoard({ exercise }) {
	const metric =
		exercise.metric ?? (exercise.type === "cardio" ? "distance" : "weight");
	...
	const primaryLabel = (e) => {
		switch (metric) {
			case "weight":
				return `${Number(e.bestWeight ?? 0)} lbs × ${e.bestReps ?? 0} reps`;
			case "time":
				return formatDuration(e.totalDurationSeconds);
			case "distance":
				return `${Number(e.totalDistance ?? 0).toFixed(2)} mi`;
			case "count":
				return `${e.bestReps}`;
			case "pace":
				return formatPaceSeconds(paceSecondsPerMile(e));
			default:
				return "";
		}
	};
	...
	const secondaryLabel = (e) => {
		...
		if (metric === "time")
			return e.totalDistance != null
				? `${Number(e.totalDistance).toFixed(2)} mi`
				: "";
		if (metric === "distance") return formatDuration(e.totalDurationSeconds);
		if (metric === "pace")
			return e.totalDistance != null
				? `${Number(e.totalDistance).toFixed(2)} mi total`
				: "";
		...
		<div className="lb-podium">
			{exercise.entries.map((e) => (
				<div
					...
					className={`lb-podium-card lb-podium-rank-${Math.min(e.rank, 4)}`}
				>
