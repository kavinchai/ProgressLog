...
describe("Cardio page", () => {
	...
	it("displays PR stats row (best pace, longest run, best week, total miles)", async () => {
		...
		expect(
			within(screen.getByTestId("stat-longest-run")).getByText(/5\.00 mi/),
		).toBeInTheDocument();
	});
	...
	it("renders time range filter buttons", async () => {
		...
		await waitFor(() => {
			...
			expect(
				within(group).getByRole("button", { name: "4W" }),
			).toBeInTheDocument();
			expect(
				within(group).getByRole("button", { name: "3M" }),
			).toBeInTheDocument();
			expect(
				within(group).getByRole("button", { name: "6M" }),
			).toBeInTheDocument();
			expect(
	...
	it("shows total miles aggregating all Running sessions", async () => {
		...
		await waitFor(() => {
			// 2.0 + 3.1 + 5.0 = 10.1
			expect(
				within(screen.getByTestId("stat-total-miles")).getByText(/10\.1 mi/),
			).toBeInTheDocument();
		});
	});
});
