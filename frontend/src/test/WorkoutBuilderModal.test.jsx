...
beforeEach(() => {
	...
	api.get.mockImplementation((url) => {
		if (url === "/workouts/exercise-names")
			return Promise.resolve({ data: ["Bench Press", "Squat", "Deadlift"] });
...
describe("WorkoutBuilderModal — rendering", () => {
	...
	it("renders Session Name field", () => {
		render(<WorkoutBuilderModal onClose={onClose} onSaved={onSaved} />);
		expect(
			screen.getByPlaceholderText(/push, pull, legs/i),
		).toBeInTheDocument();
	});
...
describe("WorkoutBuilderModal — autocomplete suggestions", () => {
	it("shows suggestions matching the typed exercise name", async () => {
		...
		await waitFor(() =>
			expect(api.get).toHaveBeenCalledWith("/workouts/exercise-names"),
		);
		await userEvent.type(
			screen.getByPlaceholderText(/exercise name/i),
			"Bench",
