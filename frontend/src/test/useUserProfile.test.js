...
describe("useUserProfile", () => {
	...
	it("saveGoals calls PUT and updates goals on success", async () => {
		...
		api.put.mockResolvedValue({
			data: { ...CUSTOM_GOALS, proteinTarget: 220 },
		});
		...
		expect(api.put).toHaveBeenCalledWith("/profile/goals", {
			...CUSTOM_GOALS,
			proteinTarget: 220,
		});
	...
	it("saveGoals sets error and re-throws on failure", async () => {
		...
		await act(async () => {
			try {
				await result.current.saveGoals(CUSTOM_GOALS);
			} catch {
				threw = true;
			}
		});
	...
	it("saving flag is true while PUT is in flight", async () => {
		...
		act(() => {
			result.current.saveGoals(CUSTOM_GOALS);
		});
		expect(result.current.saving).toBe(true);

		await act(async () => {
			resolvePut({ data: CUSTOM_GOALS });
		});
		expect(result.current.saving).toBe(false);
	});
});
