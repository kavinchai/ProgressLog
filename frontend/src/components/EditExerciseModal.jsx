...
export default function EditExerciseModal({
	...
	const initialSets = exerciseSets.map((s) => {
		...
		return {
			setNumber: s.setNumber,
			reps: String(s.reps ?? ""),
			weightLbs: String(toDisplay(s.weightLbs) ?? ""),
		};
	});
	...
	const [deletePhase, setDeletePhase] = useState(null); // null | 'confirm' | 'deleted' | 'undoing' | 'undone'
	...
	function updateSet(setIndex, field, val) {
		setSets((prev) =>
			prev.map((s, i) => (i === setIndex ? { ...s, [field]: val } : s)),
		);
	}
		...
		try {
			const mappedSets = sets.map((s) => {
				if (type === "run") {
					...
					if (s.distanceMiles !== "")
						set.distanceMiles = parseFloat(s.distanceMiles);
					return set;
				}
				if (type === "timed") {
					const totalSeconds =
						(parseInt(s.durationHours) || 0) * 3600 +
						(parseInt(s.durationMins) || 0) * 60 +
						(parseInt(s.durationSecs) || 0);
					return {
						setNumber: s.setNumber,
			...
			await api.post("/workouts/" + sessionId + "/exercises", {
				exerciseName,
				sets: mappedSets,
			});
			onSaved();
	...
	return (
		<Modal title={"Edit: " + exerciseName} onClose={onClose}>
			<form className="wbm-form" onSubmit={submit}>
				...
				<div
					className={`wbm-sets${type !== "lifting" ? " wbm-sets--cardio" : ""}`}
				>
					{type === "run" ? (
						<>
							...
							{sets.map((s, setIndex) => (
								<div key={setIndex} className="wbm-set-row wbm-set-row--cardio">
									<input
										...
										onChange={(e) =>
											updateSet(setIndex, "distanceMiles", e.target.value)
										}
									/>
									<input
										className="modal-input wbm-set-input"
										...
										onChange={(e) =>
											updateSet(setIndex, "durationMins", e.target.value)
										}
									/>
									<input
										className="modal-input wbm-set-input"
										...
										onChange={(e) =>
											updateSet(setIndex, "durationSecs", e.target.value)
										}
									/>
								</div>
							))}
							...
							{sets.map((s, setIndex) => (
								<div key={setIndex} className="wbm-set-row wbm-set-row--cardio">
									<input
										...
										onChange={(e) =>
											updateSet(setIndex, "durationHours", e.target.value)
										}
									/>
									<input
										className="modal-input wbm-set-input"
						...
						<>
							...
							{sets.map((s, setIndex) => (
								<div key={setIndex} className="wbm-set-row">
									...
									<input
										...
										value={s.weightLbs}
										onChange={(e) =>
											updateSet(setIndex, "weightLbs", e.target.value)
										}
									/>
									<input
										className="modal-input wbm-set-input"
										...
										value={s.reps}
										onChange={(e) =>
											updateSet(setIndex, "reps", e.target.value)
										}
									/>
									<button
										type="button"
							...
							<button type="button" className="btn btn-sm" onClick={addSet}>
								+ Set
							</button>
						</>
					)}
				...
				{deletePhase === "deleted" && (
					<div
						...
						style={{ justifyContent: "space-between" }}
					>
						<span style={{ fontSize: "var(--fs-sm)" }}>Exercise deleted.</span>
						<div style={{ display: "flex", gap: 8 }}>
							<button type="button" className="btn-ghost" onClick={onClose}>
								Done
							</button>
				...
				{deletePhase === "undone" && (
					<div
						...
						style={{ justifyContent: "space-between" }}
					>
						<span style={{ fontSize: "var(--fs-sm)", color: "var(--success)" }}>
							Restored!
						</span>
						<button
							type="button"
