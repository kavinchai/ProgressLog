		...
		sets: (ex.sets ?? []).map((s) => ({
			...
			durationHours:
				s.durationSeconds != null
					? String(Math.floor(s.durationSeconds / 3600))
					: "",
			durationMinutes:
				s.durationSeconds != null
					? String(Math.floor((s.durationSeconds % 3600) / 60))
					: "",
			durationSeconds:
				s.durationSeconds != null ? String(s.durationSeconds % 60) : "",
		})),
	}));
}
		...
		.map((ex) => ({
			...
			sets: ex.sets.map((s) => {
				const base = {
					...
					weightLbs: fromDisplay(s.weightLbs, unit) || 0,
				};
				if (ex.type === "run") {
...
export default function ExerciseListEditor({ exercises, onChange }) {
	...
	useEffect(() => {
		api
			.get("/workouts/exercise-names")
			.then((res) => setKnownNames(res.data))
			.catch(() => {});
	}, []);
				...
				<div className="wbm-exercises">
					{exercises.map((ex, exerciseIndex) => (
						<div key={exerciseIndex} className="wbm-exercise-block">
									...
									{suggestionFor === exerciseIndex &&
										getFilteredSuggestions(ex.exerciseName).length > 0 && (
											<ul className="wbm-suggestions">
												{getFilteredSuggestions(ex.exerciseName).map(
													(name, i) => (
														<li
															key={name}
															className={`wbm-suggestion${i === highlightedIdx ? " wbm-suggestion--active" : ""}`}
															onMouseDown={() =>
																selectSuggestion(exerciseIndex, name)
															}
														>
															{name}
														</li>
							...
							<div
								className={`wbm-sets${ex.type !== "lifting" ? " wbm-sets--cardio" : ""}`}
							>
								{ex.type === "run" ? (
									<>
										...
										{ex.sets.slice(0, 1).map((s, setIndex) => (
											<div
												key={setIndex}
												className="wbm-set-row wbm-set-row--cardio"
											>
												<input
							...
							{ex.type === "lifting" && (
								<button
									type="button"
									className="btn btn-sm wbm-add-set"
