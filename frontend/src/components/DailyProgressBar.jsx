...
export default function DailyProgressBar({ caloriesCurrent, caloriesGoal, proteinCurrent, proteinGoal, stepsCurrent, stepsGoal }) {
  ...
  const caloriesPercent = getPercentage(caloriesCurrent ?? 0, caloriesGoal ?? 1);
  ...
  function ProgressItem({ label, current, goal, percent, category, unit, exceeded }) {
    return (
      <div className="daily-progress-item">
        ...
        <div className={`daily-progress-bar-container daily-progress-bar-container--${category}`}>
          <div
            className={`daily-progress-bar daily-progress-bar--${category}`}
            style={{ width: `${percent}%` }}
          />
        </div>
        {exceeded && <span className="daily-progress-exceeded">+{Math.round(current - goal)} over</span>}
      </div>
    );
  }
